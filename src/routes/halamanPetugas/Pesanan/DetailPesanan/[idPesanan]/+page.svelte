<script>
	...
	onMount(async () => {
        ...
        try {
            const response = await ApiController({ 
                method: 'GET', 
                endpoint: `getPesananByIdEmail/${idPesanan}/${emailStored}`,
                token: token
            });
    ...
    async function updatePerluDiantar(order) {
        const token = localStorage.getItem("token");
        ...
        try {
            const response = await ApiController({
                method: 'PUT',
                endpoint: `editTungguDiantar/${order.idPesanan}`,
                token: token,
                datas: formData,
                sendForm: true
            });
    ...
    async function updateSedangDiantar(order) {
        const token = localStorage.getItem("token");
        ...
        try {
            const response = await ApiController({
                method: 'PUT',
                endpoint: `editSedangDiantar/${order.idPesanan}`,
                token: token,
                datas: formData,
                sendForm: true
            });
...
<div class="card-container">
	...
	<div class="card-info-border">
    	...
    	{#if order.statusPesanan[4].active === 'true'}
	    	<button type="button" class="button-centered" on:click={() => updatePerluDiantar(order)}>Antar Pesanan</button>
	    {:else if order.statusPesanan[5].active === 'true'}
	    	<button type="button" class="button-centered" on:click={() => updateSedangDiantar(order)}>Pesanan Selesai</button>
	    {/if}
	{/if}
	<br><br>
</div>